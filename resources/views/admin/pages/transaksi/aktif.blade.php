...
<div class="space-y-8">
                ...
                <div class="grid grid-cols-2 gap-4">
                    ...
                    <div>
                        ...
                        <select name="karyawan_id" id="tambah_karyawan_id" required class="w-full px-4 py-2 bg-gray-50/50 border border-gray-300 rounded-xl focus:outline-none focus:ring-2 focus:ring-[#825449]/20 focus:border-[#825449] transition-all cursor-pointer">
                            <option value="">-- Pilih Karyawan --</option>
                            @foreach(\App\Models\Karyawan::whereDoesntHave('transaksis', function($q) { $q->where('status', 'dikerjakan'); })->get() as $karyawan)
                                <option value="{{ $karyawan->id }}" data-gender="{{ $karyawan->jenis_kelamin }}" style="display:none;">
                                    {{ $karyawan->nama }}
                                </option>
                            @endforeach
                        </select>
    ...
    <script id="users-data" type="application/json">
        {!! \App\Models\User::whereIn('role', ['pelanggan', 'user'])->get()->map(function($user) {
            return [
                'id' => (string) $user->id,
                'name' => $user->name,
                'email' => $user->email,
